/**
 * Mova Cloud authentication and communication module.
 * Implements login, device discovery, and MQTT communication with Mova cloud.
 *
 * @file movaCloud.ts
 * @license Apache-2.0
 */
...
import {
  ...
  MOVA_AUTH_HEADER,
  MOVA_CLIENT_ID,
  MOVA_DEVICE_LIST_ENDPOINT,
  MOVA_DOWNLOAD_ENDPOINT,
  MOVA_GET_DEVICE_DATA_ENDPOINT,
  MOVA_OSS_DOWNLOAD_ENDPOINT,
  MOVA_PASSWORD_SALT,
...
export class MovaCloudProtocol {
  ...
  /**
   * Login to Mova Cloud with credentials.
   ...
   * @param country
   */
  async login(username: string, password: string, country: MovaCountry): Promise<AuthResult> {
    ...
    try {
      ...
      // CN region keeps Dreame's original regional-routing header name.
      if (country === 'cn') {
  ...
  private async apiCall<T>(endpoint: string, method: 'GET' | 'POST' = 'GET', body?: Record<string, unknown>): Promise<T | null> {
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': userAgent,
      'Authorization': MOVA_AUTH_HEADER, // Basic auth required for all API calls
      'Tenant-Id': tenantId,
  ...
  async getDevices(): Promise<MovaDevice[]> {
    ...
    // Call device list endpoint with pagination, then retry the MOVAhome no-body shape if needed.
    let response = await this.apiCall<DeviceListResponse>(MOVA_DEVICE_LIST_ENDPOINT, 'POST', {
      page: 1,
      pageSize: 100,
    });

    if (!response?.data?.page?.records) {
      response = await this.apiCall<DeviceListResponse>(MOVA_DEVICE_LIST_ENDPOINT, 'POST');
    }
    ...
    // Filter to only supported vacuum models (or all vacuum models if model starts with 'mova.vacuum')
  ...
  async getDeviceProperties(did: string): Promise<DeviceStatus | null> {
    ...
    try {
      ...
      const status: DeviceStatus = {
        state: (getValue(MIOT_PROPERTIES.operatingMode.siid, MIOT_PROPERTIES.operatingMode.piid) as MovaState) ?? MovaState.Unknown,
        status: (getValue(MIOT_PROPERTIES.deviceStatus.siid, MIOT_PROPERTIES.deviceStatus.piid) as MovaStatus) ?? MovaStatus.Unknown,
        battery: (getValue(MIOT_PROPERTIES.batteryLevel.siid, MIOT_PROPERTIES.batteryLevel.piid) as number) ?? 0,
        fanSpeed: (getValue(MIOT_PROPERTIES.suctionLevel.siid, MIOT_PROPERTIES.suctionLevel.piid) as MovaFanSpeed) ?? MovaFanSpeed.Standard,
        waterFlow: (getValue(MIOT_PROPERTIES.waterFlow.siid, MIOT_PROPERTIES.waterFlow.piid) as MovaWaterFlow) ?? MovaWaterFlow.Medium,
        cleaningMode: getValue(MIOT_PROPERTIES.cleaningMode.siid, MIOT_PROPERTIES.cleaningMode.piid) as MovaCleaningMode | undefined,
        errorCode: (getValue(MIOT_PROPERTIES.deviceFault.siid, MIOT_PROPERTIES.deviceFault.piid) as MovaErrorCode) ?? MovaErrorCode.None,
  ...
  private async fetchMapFromCloudStorage(did: string, objectPath: string): Promise<void> {
    try {
      ...
      for (const key of mapKeys) {
        const response = await this.apiCall<DeviceDataResponse>(MOVA_GET_DEVICE_DATA_ENDPOINT, 'POST', {
          did,
          model: [key],
        });
      ...
      const ossResponse = await this.apiCall<FileUrlResponse>(MOVA_OSS_DOWNLOAD_ENDPOINT, 'POST', {
        did,
        filename: objectPath,
        model: deviceModel,
      });
      ...
      const fileResponse = await this.apiCall<FileUrlResponse>(MOVA_DOWNLOAD_ENDPOINT, 'POST', {
        did,
        filename: objectPath,
        model: deviceModel,
      });
   ...
   * @param device
   */
  async connectMqtt(device: MovaDevice): Promise<boolean> {
    ...
    try {
      ...
      const client = mqtt.connect({
        ...
        clientId,
        rejectUnauthorized: false, // Required for Mova's self-signed certificates
  ...
  private handleMqttMessage(did: string, _topic: string, payload: string): void {
    try {
      ...
      // Mova wraps the actual message in a "data" field
  ...
  private parseStatusFromMqtt(did: string, params: unknown[]): DeviceStatus | null {
    try {
      ...
      const newStatus: DeviceStatus = {
        state: (getValue(MIOT_PROPERTIES.operatingMode.siid, MIOT_PROPERTIES.operatingMode.piid) as MovaState) ?? cached.state,
        status: (getValue(MIOT_PROPERTIES.deviceStatus.siid, MIOT_PROPERTIES.deviceStatus.piid) as MovaStatus) ?? cached.status,
        battery: (getValue(MIOT_PROPERTIES.batteryLevel.siid, MIOT_PROPERTIES.batteryLevel.piid) as number) ?? cached.battery,
        fanSpeed: (getValue(MIOT_PROPERTIES.suctionLevel.siid, MIOT_PROPERTIES.suctionLevel.piid) as MovaFanSpeed) ?? cached.fanSpeed,
        waterFlow: (getValue(MIOT_PROPERTIES.waterFlow.siid, MIOT_PROPERTIES.waterFlow.piid) as MovaWaterFlow) ?? cached.waterFlow,
        cleaningMode: (getValue(MIOT_PROPERTIES.cleaningMode.siid, MIOT_PROPERTIES.cleaningMode.piid) as MovaCleaningMode) ?? cached.cleaningMode,
        errorCode: (getValue(MIOT_PROPERTIES.deviceFault.siid, MIOT_PROPERTIES.deviceFault.piid) as MovaErrorCode) ?? cached.errorCode,
      };
   ...
   * Always uses cloud API since Mova MQTT is receive-only for clients.
  ...
  async sendCommand(did: string, method: string, params: unknown[] = []): Promise<unknown> {
    // Always use cloud API - Mova MQTT broker is receive-only for clients
   ...
   * Uses the Mova Cloud API format with wrapped data field.
   *
   * @param did
   * @param method
   * @param params
  ...
  private async sendCloudCommand(did: string, method: string, params: unknown[]): Promise<unknown> {
    ...
    // Mova cloud API format: command wrapped in "data" field
    const requestBody = {
      did,
      id,
      data: {
    ...
    const response = await this.apiCall<ActionResponse>(getMovaSendCommandEndpoint(this.deviceBindDomains.get(did)), 'POST', requestBody);
   ...
   *
   * @param did - Device ID
   * @param expectedStates - Array of acceptable MovaState values
   * @param expectedStatuses - Array of acceptable MovaStatus values (optional)
  ...
  private async confirmCommandSuccess(
    did: string,
    expectedStates: MovaState[],
    expectedStatuses?: MovaStatus[],
    ...
    try {
      ...
      if (confirm) {
        return this.confirmCommandSuccess(
          did,
          [MovaState.Returning, MovaState.GoCharging, MovaState.Charging, MovaState.Idle],
          [MovaStatus.BackHome, MovaStatus.Charging, MovaStatus.Standby],
        );
      }
      return true;
    } catch (error) {
   ...
   * @param cleaningMode - MOVA S70 raw mode: 2=vacuum only, 1=mop only, 0=vacuum+mop, 3=vacuum then mop
   */
  async cleanRooms(
    did: string,
    roomIds: number[],
    repeat: number = 1,
    cleaningMode: MovaCleaningMode = MovaCleaningMode.SweepingAndMopping,
  ...
  /**
   * Set the global cleaning mode.
   *
   * @param did - Device ID
   * @param cleaningMode - MOVA S70 raw mode: 2=vacuum only, 1=mop only, 0=vacuum+mop, 3=vacuum then mop
   */
  async setCleaningMode(did: string, cleaningMode: MovaCleaningMode): Promise<void> {
  ...
  private roomWaterLevelForCleaningMode(cleaningMode: MovaCleaningMode): number {
    // S70 Roller raw mapping: 2=vacuum only, 0=vacuum+mop.
